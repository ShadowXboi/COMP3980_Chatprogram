...
// Global flag to indicate socket closing
static volatile sig_atomic_t socket_closing = 0;
...
int main(int argc, char *argv[]) {
    // Flag to handle server and client
    int opt;
    opterr = 0;
    ...
    while ((opt = getopt(argc, argv, ":ac")) != -1) {
        switch (opt) {
            ...
            default:
                ...
                usage(argv[0], EXIT_FAILURE, "Invalid usage");
        }
    }
    ...
    // Check for two additional arguments
    if (c_flag && argc - optind == 2) {
        ...
        socket_closing = 1;  // Set the flag to indicate socket is about to close
        socket_close(sockfd);
...
//        }
        ...
        pthread_join(server_thread, NULL);  // Wait for the server input thread to finish
        socket_close(sockfd);
